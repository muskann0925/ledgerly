      ...
      <DialogContent className="max-w-md p-6 rounded-2xl bg-white dark:bg-[#111827] border-slate-200 dark:border-slate-800">
        <DialogHeader className="space-y-2">
          ...
          <DialogTitle className="text-lg font-bold text-slate-900 dark:text-white">
            Soft Delete Invoice {invoice.number}?
          </DialogTitle>
          <DialogDescription className="text-xs text-slate-500 dark:text-slate-400">
            This will mark invoice <span className="font-semibold text-slate-700 dark:text-slate-200">{invoice.number}</span> for <span className="font-semibold text-slate-700 dark:text-slate-200">{invoice.client?.companyName}</span> as soft-deleted. You can restore it later if needed.
          </DialogDescription>
        </DialogHeader>
