      ...
      <DialogContent className="max-w-md p-6 rounded-2xl bg-white dark:bg-[#111827] border-slate-200 dark:border-slate-800">
        <DialogHeader className="space-y-2">
          ...
          <DialogDescription className="text-xs text-slate-500 dark:text-slate-400">
            This will mark payment of <span className="font-semibold text-slate-700 dark:text-slate-200">{formatCurrency(payment.amount)}</span> for invoice <span className="font-semibold text-slate-700 dark:text-slate-200">{payment.invoice?.number}</span> as soft-deleted and automatically recalculate invoice balance due.
          </DialogDescription>
        </DialogHeader>
